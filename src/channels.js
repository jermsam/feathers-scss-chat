module.exports = function(app) {
  ...
  app.on('login', async(authResult, { connection }) => {
    ...
    if(connection) {
      ...
      const authId =authResult.user._id;
      await app.service('users').patch(authId,{online:true});
  ...
  // when you logout join anonymous user

  app.on('logout', async(authResult, { connection }) => {

    if(connection) {
    // await  app.service('online-users').remove(payload.user._id);
      // Join the channels a logged out connection should be in
      const authId =authResult.user._id;
      await app.service('users').patch(authId,{online:false});
      app.channel('anonymous').join(connection);
    }
  });
  ...
  app.service('messages').publish('created', (data, context)=> {
    ...
    if(data.to){
      ...
      return app.channel(app.channels).filter(connection =>
        (connection.user._id === context.params.user._id || connection.user._id ===data.to)
      );
    }else{
